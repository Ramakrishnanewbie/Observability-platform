...
export default function DashboardPage() {
  const { selectedHost, isGCP } = useDatasource();
  ...
  const [gcpStorageData, setGcpStorage] = useState<any[]>([]);
  const [gcpApiData, setGcpApi] = useState<any[]>([]);
  const [latest, setLatest] = useState<Record<string, any>>({});
  const [prevLatest, setPrevLatest] = useState<Record<string, any>>({});
  const [hosts, setHosts] = useState<string[]>([]);  // kept for host count display
  const [connected, setConnected] = useState(false);
  const [lastUpdate, setLastUpdate] = useState<Date | null>(null);
  const [timeRange, setTimeRange] = useState(60);
  const [initialLoaded, setInitialLoaded] = useState(false);

  // Detect if selected host is a GCP host — now driven by context
  // isGCP and selectedHost come from useDatasource() above

  const fetchData = useCallback(async () => {
    try {
      const h = selectedHost || undefined;
      const [c, m, d, mb, gcpSt, gcpApi, l, hList] = await Promise.all([
        ...
        observoApi.getTimeseries("gcp.storage.total_bytes", timeRange, h),
        observoApi.getTimeseries("gcp.api.request_count", timeRange, h),
        observoApi.getLatest(h),
        observoApi.getHosts().catch(() => []),
      ]);
      setCpu(c || []); setMem(m || []); setDisk(d || []); setMemBytes(mb || []);
      setGcpStorage(gcpSt || []); setGcpApi(gcpApi || []);
      setHosts(hList || []);
  ...
  const gcpStorageSpark = useMemo(() => gcpStorageData.map(d => ({ v: d.value })), [gcpStorageData]);
  const gcpApiSpark = useMemo(() => gcpApiData.map(d => ({ v: d.value })), [gcpApiData]);
      ...
      <header className="h-12 flex items-center justify-between px-6 border-b border-border bg-card/80 backdrop-blur-sm shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            {connected ? <Wifi className="h-3.5 w-3.5 text-emerald-500" /> : <WifiOff className="h-3.5 w-3.5 text-red-400" />}
        ...
        {isGCP ? (
          <>
            <div className="flex items-center gap-2 mb-4">
              <span className="text-[11px] font-semibold text-blue-400 uppercase tracking-wider">☁ GCP Cloud Monitoring</span>
              <span className="text-[10px] text-muted-foreground/40">· {selectedHost || "All GCP hosts"}</span>
            </div>
            <div className="grid grid-cols-4 gap-3 mb-4">
              <KPICard
                label="Storage Total" value={latest["gcp.storage.total_bytes"]?.value ?? null}
                prevValue={prevLatest["gcp.storage.total_bytes"]?.value}
                unit="bytes" icon={HardDrive} color="#3B82F6" sparkData={gcpStorageSpark} loaded={initialLoaded}
              />
              <KPICard
                label="API Requests" value={latest["gcp.api.request_count"]?.value ?? null}
                prevValue={prevLatest["gcp.api.request_count"]?.value}
                unit="count" icon={Activity} color="#22C55E" sparkData={gcpApiSpark} loaded={initialLoaded}
              />
              <KPICard
                label="CPU Usage" value={latest["cpu.usage_percent"]?.value ?? null}
                prevValue={prevLatest["cpu.usage_percent"]?.value}
                unit="percent" icon={Cpu} color="#A855F7" sparkData={cpuSpark} loaded={initialLoaded}
              />
              <KPICard
                label="Memory Usage" value={latest["memory.usage_percent"]?.value ?? null}
                prevValue={prevLatest["memory.usage_percent"]?.value}
                unit="percent" icon={MemoryStick} color="#8B5CF6" sparkData={memSpark} loaded={initialLoaded}
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <MetricChart title="GCP Storage Total Bytes" data={gcpStorageData} color="#3B82F6" height={240} loaded={initialLoaded} />
              <MetricChart title="GCP API Request Count" data={gcpApiData} color="#22C55E" height={240} loaded={initialLoaded} />
            </div>
          </>
        ) : (
          <>
              ...
              <div className="bg-card border border-border rounded-xl p-4 flex flex-col shadow-sm">
                ...
                <div className="mt-3 pt-3 border-t border-border/30">
                  <div className="flex items-center justify-between text-[10px] text-muted-foreground/50">
                    ...
                    <span className="flex items-center gap-1"><Clock className="h-3 w-3" />Last {timeRange}min</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex-1 overflow-auto px-3 pb-3">
                  ...
                  {cpuData.length === 0 && (
                    <div className="flex items-center justify-center h-full text-muted-foreground/30 text-xs">
                      {initialLoaded ? "No data yet" : <><Loader2 className="h-4 w-4 animate-spin mr-2" />Loading…</>}
                    </div>
                  )}
                </div>
              </div>
